...
```
composer require highjin/laravel-queue-monitor
```
...
```
php artisan vendor:publish --provider="highjin\QueueMonitor\Providers\QueueMonitorProvider"  --tag=config --tag=migrations
```
